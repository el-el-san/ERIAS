...
export default class Planner implements PlannerInterface {
    ...
    try {
      ...
      try {
       ...
       try {
         ...
         logError(`Failed to write Plan.md: ${(writeError as Error).message}`);
  ...
  public async adjustPlan(task: ProjectTask, feedback: string): Promise<DevelopmentPlan> {
    ...
    try {
      ...
      try {
        ...
        logError(`Error parsing adjusted plan JSON: ${(error as Error).message}`);
      ...
      logError(`Error adjusting plan: ${(error as Error).message}`);
      throw error;
    } finally {
      // 登録したツールを解除
      toolRegistry.clearTools();
  ...
  public async refactorPlan(task: ProjectTask, processingPrompt: string): Promise<DevelopmentPlan> {
    ...
    try {
      ...
      try {
        ...
        logError(`Error parsing refactored plan JSON: ${(error as Error).message}`);
      ...
      logError(`Error refactoring plan: ${(error as Error).message}`);
      throw error;
    } finally {
      // 登録したツールを解除
      toolRegistry.clearTools();
  ...
  private _generatePlanMarkdown(plan: DevelopmentPlan): string {
    ...
    if (plan.dependencies?.production && plan.dependencies.production.length > 0) {
      plan.dependencies.production.forEach((dep: string) => markdown += `- ${dep}\n`);
    } else {
      markdown += `N/A\n`;
    }
    markdown += `### Development\n`;
    if (plan.dependencies?.development && plan.dependencies.development.length > 0) {
      plan.dependencies.development.forEach((dep: string) => markdown += `- ${dep}\n`);
    } else {
      markdown += `N/A\n`;
    }
    markdown += `\n`;
    ...
    if (plan.files && plan.files.length > 0) {
      plan.files.forEach((file: FileInfo) => {
        markdown += `- **${file.path}**\n`;
