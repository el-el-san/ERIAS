...
import { ProjectTask, ErrorInfo, Debugger as DebuggerInterface } from '../types/agentTypes';
...
export default class Debugger implements DebuggerInterface {
  ...
  public async fixError(task: ProjectTask, errorInfo: ErrorInfo): Promise<boolean> {
    ...
    try {
      ...
      logError(error, `Error fixing bug: ${(error as Error).message}`);
      return false;
    } finally {
      // 登録したツールを解除
  ...
  private async findRelatedFile(task: ProjectTask, errorInfo: ErrorInfo): Promise<string | undefined> {
    ...
    try {
      ...
      logError(error, `Error finding related file: ${(error as Error).message}`);
      return undefined;
    }
  }
