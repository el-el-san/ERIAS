...
export class FeedbackHandler {
  ...
  public async processQueuedFeedbacks(
    ...
    const combinedFeedback = pendingFeedbacks
      .map((f: UserFeedback, i: number) => `${i+1}. ${f.content}`)
      .join('\n');
...
これらの指示を考慮して${context}を行ってください。
  ...
  public async processCriticalFeedbacks(
    ...
    const queue = task.feedbackQueue;
    const criticalFeedbacks = queue?.feedbacks?.filter((f: UserFeedback) => f.status === 'pending' && f.urgency === 'critical') ?? [];
...
何を変更すべきか、どのファイルを修正・追加すべきか具体的に説明してください。
      ...
      if (updatedPlan && task.plan) {
        ...
        for (const file of updatedPlan.files) {
          const existingFile = task.plan.files.find((f: FileInfo) => f.path === file.path);
          if (existingFile) {
  ...
  public async recodeBasedOnFeedback(
    ...
    if (!task.plan) {
      logError('No plan available for recoding');
      return;
    }
    
    // 計画から変更が必要なファイルを特定
    const filesToRecode = task.plan.files.filter((file: FileInfo) => file.needsUpdate);
    ...
    for (let i = 0; i < filesToRecode.length; i++) {
      ...
      try {
        ...
        logError(error, `Error regenerating file ${fileInfo.path}: ${(error as Error).message}`);
        fileInfo.status = 'error';
      }
    }
    
    // 新しいファイルの追加
    const newFiles = task.plan.files.filter((file: FileInfo) => file.status === 'pending' && !file.content);
    ...
    for (let i = 0; i < newFiles.length; i++) {
      ...
      try {
        ...
        logError(error, `Error generating new file ${fileInfo.path}: ${(error as Error).message}`);
        fileInfo.status = 'error';
      }
    }
