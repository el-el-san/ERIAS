  ...
  async generateResponse(message: string, target: NotificationTarget): Promise<string> {
    ...
    try {
      ...
      try {
        ...
        logError(`Gemini API error: ${(llmError as Error).message}`);
        if (llmError instanceof Error && llmError.stack) {
          logError(`エラースタック: ${llmError.stack}`);
        }
        
        return this.generateFallbackResponse(target);
      }
    } catch (error) {
      logError(`Error in generateResponse: ${(error as Error).message}`);
      if (error instanceof Error && error.stack) {
        logError(`エラースタック: ${error.stack}`);
      }
