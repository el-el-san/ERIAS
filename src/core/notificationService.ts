...
import { MessageContent } from '../integrations/platforms/types';
import { NotificationTarget, PlatformType } from '../types/agentTypes';
...
export class NotificationService {
  ...
  async sendNotification(target: NotificationTarget, payload: NotificationPayload): Promise<string | null> {
    try {
      ...
      logError(error, `Failed to send notification to ${target.platformType}:`);
      return null;
    }
  }
  ...
  async updateNotification(target: NotificationTarget, messageId: string, payload: NotificationPayload): Promise<boolean> {
    try {
      ...
      logError(error, `Failed to update notification on ${target.platformType}:`);
      return false;
    }
  }
