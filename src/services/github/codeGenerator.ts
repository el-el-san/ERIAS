...
export class CodeGenerator {
  ...
  private async determineRequiredFiles(): Promise<Array<{
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logger.error(`ファイル構造決定中にエラーが発生: ${(error as { message?: string }).message}`);
      } else {
        logger.error('ファイル構造決定中にエラーが発生: 不明なエラー');
      }
  ...
  private async generateRequiredFiles(requiredFiles: Array<{
    ...
    for (const fileInfo of requiredFiles) {
      try {
        ...
        if (typeof error === 'object' && error !== null && 'message' in error) {
          logger.error(`ファイル生成中にエラーが発生 (${fileInfo.path}): ${(error as { message?: string }).message}`);
        } else {
          logger.error(`ファイル生成中にエラーが発生 (${fileInfo.path}): 不明なエラー`);
        }
  ...
  private async generateTypeScriptFile(
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logger.error(`TypeScriptファイル生成エラー: ${(error as { message?: string }).message}`);
      } else {
        logger.error('TypeScriptファイル生成エラー: 不明なエラー');
      }
      throw error;
    }
  }
  ...
  private async generatePythonFile(
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logger.error(`Pythonファイル生成エラー: ${(error as { message?: string }).message}`);
      } else {
        logger.error('Pythonファイル生成エラー: 不明なエラー');
      }
      throw error;
    }
  }
  ...
  private async generateRecommendations(
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logger.error(`レコメンデーション生成エラー: ${(error as { message?: string }).message}`);
      } else {
        logger.error('レコメンデーション生成エラー: 不明なエラー');
      }
