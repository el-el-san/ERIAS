...
export class RepositoryAnalyzer {
  ...
  public async analyzeRepository(): Promise<RepoAnalysisResult> {
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(`リポジトリ分析中にエラーが発生: ${(error as { message?: string }).message}`);
        throw new Error(`リポジトリ分析に失敗しました: ${(error as { message?: string }).message}`);
      } else {
        logError('リポジトリ分析中にエラーが発生: 不明なエラー');
  ...
  private async getRepositoryLanguages(): Promise<Record<string, number>> {
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(`言語統計の取得に失敗: ${(error as { message?: string }).message}`);
      } else {
        logError('言語統計の取得に失敗: 不明なエラー');
      }
      return {};
    }
  }
  ...
  private async getFileStructure(dirPath: string = ''): Promise<FileNode[]> {
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(`ファイル構造の取得に失敗 (${dirPath}): ${(error as { message?: string }).message}`);
      } else {
        logError(`ファイル構造の取得に失敗 (${dirPath}): 不明なエラー`);
      }
      return [];
    }
  }
  ...
  private async analyzeDependencies(): Promise<DependencyInfo> {
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(`依存関係の分析に失敗: ${(error as { message?: string }).message}`);
      } else {
        logError('依存関係の分析に失敗: 不明なエラー');
      }
      return result;
    }
  }
