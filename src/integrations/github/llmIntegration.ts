...
export class LLMIntegration {
    ...
    try {
      ...
      try {
        ...
        if (typeof parseError === 'object' && parseError !== null && 'message' in parseError) {
          logError(parseError, `応答の解析に失敗: ${(parseError as { message?: string }).message}`);
          throw new Error(`応答の解析に失敗: ${(parseError as { message?: string }).message}`);
        } else {
          logError(parseError, '応答の解析に失敗: 不明なエラー');
          throw new Error('応答の解析に失敗: 不明なエラー');
        }
      }
    } catch (error: unknown) {
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(error, `タスク分析中にエラーが発生: ${(error as { message?: string }).message}`);
      } else {
        logError(error, 'タスク分析中にエラーが発生: 不明なエラー');
      }
      throw error;
    }
  }
  ...
  public async generateRecommendations(
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(error, `レコメンデーション生成中にエラーが発生: ${(error as { message?: string }).message}`);
      } else {
        logError(error, 'レコメンデーション生成中にエラーが発生: 不明なエラー');
      }
      throw error;
    }
  }
...
...`;
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(error, `PR説明生成中にエラーが発生: ${(error as { message?: string }).message}`);
      } else {
        logError(error, 'PR説明生成中にエラーが発生: 不明なエラー');
      }
...
\`\`\``;
        ...
        if (jsonMatch) {
          try {
            ...
            if (typeof error === 'object' && error !== null && 'message' in error) {
              logError(error, `レビューコメントのJSON解析に失敗: ${(error as { message?: string }).message}`);
            } else {
              logError(error, 'レビューコメントのJSON解析に失敗: 不明なエラー');
            }
          }
        }
      }
      ...
      if (typeof error === 'object' && error !== null && 'message' in error) {
        logError(`PRレビューコメント生成中にエラーが発生: ${(error as { message?: string }).message}`);
      } else {
        logError('PRレビューコメント生成中にエラーが発生: 不明なエラー');
      }
      return [];
    }
  }
