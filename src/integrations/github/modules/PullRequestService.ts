...
export class PullRequestService extends GitHubServiceBase {
  ...
  public async createPullRequest(
    ...
    try {
      ...
      try {
        ...
        logError(repoError, `リポジトリ確認エラー: ${this.getErrorMessage(repoError)}`);
      ...
      logError(error, `プルリクエスト作成中にエラーが発生: ${errorMsg}`);
      
      // エラーメッセージがブランチ履歴の問題を示している場合
      if (errorMsg.includes('no history in common') || errorMsg.includes('no common ancestor')) {
        logError(error, `ブランチの履歴に問題があります。ベースブランチ(${baseBranch})とヘッドブランチ(${headBranch})に共通の履歴がありません。`);
  ...
  public async reviewPullRequest(prNumber: number): Promise<void> {
    try {
      ...
      const errorMsg = this.getErrorMessage(error);
      logError(error, `プルリクエストレビュー中にエラーが発生: ${errorMsg}`);
  ...
  public async mergePullRequest(
    ...
    try {
      ...
      const errorMsg = this.getErrorMessage(error);
      logError(error, `プルリクエストマージ中にエラーが発生: ${errorMsg}`);
      return false;
    }
  }
  ...
  public async getPullRequestStatus(
    ...
    try {
      ...
      const errorMsg = this.getErrorMessage(error);
      logError(error, `PR状態取得中にエラーが発生: ${errorMsg}`);
